...
_build_libmaxminddb() {
    ...
    echo
    sleep 2
    tar -Jcvf /tmp/libmaxminddb-git-1.el8.x86_64.tar.xz *
    echo
    sleep 2
    cd /tmp
    rm -fr "${_tmp_dir}"
...
_build_brotli() {
    ...
    echo
    sleep 2
    tar -Jcvf /tmp/brotli-git-1.el8.x86_64.tar.xz *
    echo
    sleep 2
    cd /tmp
    rm -fr "${_tmp_dir}"
...
_build_libmaxminddb
_build_brotli
tar -xof /tmp/libmaxminddb-*.el8.x86_64.tar.xz -C /
tar -xof /tmp/brotli-*.el8.x86_64.tar.xz -C /
sleep 1
/sbin/ldconfig >/dev/null 2>&1
rm -f /tmp/libmaxminddb-*.el8.x86_64.tar.xz /tmp/brotli-*.el8.x86_64.tar.xz
...
if [[ -f /usr/lib/rpm/redhat/redhat-hardened-ld ]]; then
    _suffix_ld="$(head /dev/urandom | sha512sum | cut -c3-10)"
    cp -f /usr/lib/rpm/redhat/redhat-hardened-ld /usr/lib/rpm/redhat/redhat-hardened-ld.${_suffix_ld}
    cat /dev/null > /usr/lib/rpm/redhat/redhat-hardened-ld
fi
if [[ -f /usr/lib/rpm/redhat/redhat-hardened-cc1 ]]; then
    _suffix_cc1="$(head /dev/urandom | sha512sum | cut -c3-10)"
...
#yum makecache && yum install -y glibc libxml2 libxslt gd perl-libs perl
...
echo
sleep 2
tar -Jcvf /tmp/gws-v"${_vmajor}.${_vminor}.${_vpatch}"-1.el8.x86_64.tar.xz *
echo
sleep 2

cat /usr/lib/rpm/redhat/redhat-hardened-ld.${_suffix_ld} > /usr/lib/rpm/redhat/redhat-hardened-ld
rm -f /usr/lib/rpm/redhat/redhat-hardened-ld.${_suffix_ld}
cat /usr/lib/rpm/redhat/redhat-hardened-cc1.${_suffix_cc1} > /usr/lib/rpm/redhat/redhat-hardened-cc1
rm -f /usr/lib/rpm/redhat/redhat-hardened-cc1.${_suffix_cc1}
